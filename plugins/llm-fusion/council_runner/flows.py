...
def _write_handoff(template: str, brief: str, answers, paths: RunPaths, mode: str,
                   context: str = "") -> None:
    body = render(template, BRIEF=brief, ANSWERS=answers_block(answers), CONTEXT=context)
    preamble = (
...
async def run_advise(roster, project_root, login_path, prompts, brief, runs_base, backend,
                     context: str = "", context_files: list[str] | None = None) -> RunPaths:
    ...
    if context:
        ...
        write_meta(paths, context_files=context_files or [], context_chars=len(context))

    round1_template = render(prompts["round1_advise"], CONTEXT=context)
    results = await run_round1(roster, roster.advise_agents, project_root, login_path,
                               paths, round1_template, brief)
    ...
    if backend == "auto":
        ...
        jprompt = render(prompts["judge_advise"], BRIEF=brief, ANSWERS=answers_block(answers), CONTEXT=context)
...
async def run_execute(roster, project_root, login_path, prompts, brief, runs_base, backend, workspace,
    ...
    round1_template = render(prompts["round1_execute"], CONTEXT=context)
    ...
    paths.judge.mkdir(parents=True, exist_ok=True)
    jprompt = render(prompts["judge_execute_spec"], BRIEF=brief, ANSWERS=answers_block(answers), CONTEXT=context)
    (paths.judge / "prompt.md").write_text(jprompt, encoding="utf-8")
