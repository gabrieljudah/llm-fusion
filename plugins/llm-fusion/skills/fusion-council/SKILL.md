...
1. **The council is fixed: three frontier models, one per vendor** — **Claude Fable 5.1**, **Codex GPT-6 Astra**, **Grok 4.6**. Do not ask the user to pick models; all three run every time (the runner requires ≥3 models across ≥2 providers, so the full roster is also the minimum).

2. **Run the council** (advise mode, you judge). Locate the bundled runner, then run it:
   ```bash
...
The full advise council is **7 lenses across the 3 models** (architect, pragmatist, skeptic, first-principles, operator, user-advocate, realist). Lenses may be re-paired only within the three routes shown by `python3 -m council_runner --list-models`; unsupported custom model IDs fail loudly.
...
- Doctor check before a run if unsure: `cd "$PLUGIN" && python3 -m council_runner --doctor` (verifies the member has claude + codex + grok installed + authenticated; GPT-6 Astra needs codex ≥ 0.153).
